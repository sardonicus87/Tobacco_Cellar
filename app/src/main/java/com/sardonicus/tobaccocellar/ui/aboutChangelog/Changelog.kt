...
val changelogEntries = listOf(
    ...
    ChangelogEntryData(
        ...
        improvements = listOf(
            "Plaintext Screen, the formatting guide list of placeholders is now more flexible to " +
                    "screen sizes (spreads the list into additional columns).",
            "Other minor UI/UX fixes/tweaks (mostly spacing and color corrections).",
            "General performance and reliability improvements."
        ),
        bugFixes = listOf(
            "Fixed Multi-Device sync bug related to network monitor that could result in random " +
                    "forced-closures of the app and issues with sync.",
            "Other minor bug fixes."
        )
    ),
    ...
    ChangelogEntryData(
        versionCode = 60,
        versionNumber = "5.3.0",
        buildDate = "4 Sep, 2026",
        changes = listOf(
            "Plaintext Export major changes: added a new option in the formatting tab to display " +
                    "the text as a list of Entries or as Tins. Listing as Tins will explode all " +
                    "tins for an entry into individual entries (and items with no tins will be a " +
                    "single entry). Sorting/sub-sorting options have been expanded, and all " +
                    "options are now generated by whether it's possible given the current filters " +
                    "rather than being based on format string (though tin sorting is only " +
                    "available when listing as tins)."
        ),
        improvements = listOf(
            "Dual Pane Layouts and Foldable Devices: the \"landscape only\" option now considers " +
                    "landscape orientation to be when the hinge is vertical (regardless of actual " +
                    "height/width ratio).",
            "Add/Edit Entry screen, small devices, collapse the top app bar when the keyboard " +
                    "is open in landscape.",
            "System back handling improved in dual pane layouts.",
            "Text selections now preserved on device rotation.",
            "Filtering: selected brands box now dynamically fills with as many chips as it can " +
                    "hold in the available space, rather than only holding up to 5.",
            "Focus clearing improvements: on any screen, most tab or navigation taps while a " +
                    "text field or copyable text is selected will instead clear focus (a second " +
                    "tap takes the normal action).",
            "Cross-Device Sync, the 12-hour check now updates with changes to your \"Allow Mobile" +
                    "\" preferences (previously was set permanently as whatever your mobile data " +
                    "setting was at the time it was first set up).",
            "Backup/Restore: device sync login state now saved/restored in manual backup process.",
            "Various other minor UI/UX fixes and refinements (e.g. in landscape with status bars " +
                    "hidden, prevent opening filter sheet from showing them).",
            "Efficiency and stability improvements."
        ),
        bugFixes = listOf(
            "Large Screen Add/Edit Entry screen: fixed bug if two column view is restricted to " +
                    "landscape only, tab position is lost when rotating device.",
            "Devices using single-pane layouts: fixed bug where rotation to landscape on About " +
                    "Screen navigated to Settings Screen in some cases.",
            "Plaintext Export bugs fixed: sorting menu positioning wrong on some devices, possible " +
                    "out-of-memory crash when entering format string in certain ways, format " +
                    "string not returning exactly as typed if it was detecting placeholders, " +
                    "some placeholders not working unless surrounded by spaces."
        ),
        releaseNotes = listOf(
            "Several performance enhancements and UI/UX improvements (especially more adaptive " +
                    "layouts for various screen sizes and types). I tried to thoroughly test " +
                    "everything, but if a bug has been introduced, do not hesitate to e-mail me. " +
